...
import { colorUsage, createProject, DEFAULT_CREATION_SIZE, MAX_CREATION_SIZE, MIN_CREATION_SIZE, parsePalette } from "./editor-core";
...
import { expectedPdfPageCount, recommendedPdfLayout, type PdfContentMode, type PdfLayoutMode } from "./pdf-export";
...
function ExportModal({ project, total, grid, transparent, scale, setGrid, setTransparent, setScale, onClose }: ExportModalProps) {
  ...
  const [pdfLayout, setPdfLayout] = useState<PdfLayoutMode>(() => recommendedPdfLayout(project));
  const pdfPages = expectedPdfPageCount(project, pdfMode, pdfLayout);
  const pdfCells = pdfMode === "progress" || !project.guideCells ? project.cells : project.guideCells;
  const pdfUsedColors = new Set([...pdfCells].filter(Boolean)).size;
  return <div className="modal-backdrop" role="presentation"><section className="modal export-modal" role="dialog" aria-modal="true"><header><div><span className="eyebrow">OUTPUT BAY / 03</span><h2>完成并导出</h2></div><button className="icon-button" onClick={onClose}>×</button></header><div className="export-cards">
    ...
    <article><span className="export-glyph">▤</span><h3>PDF 拼豆图纸</h3><p>{pdfLayout === "overview" ? "一页展示完整图案、坐标与材料统计，适合单板和简单作品。" : "总览页搭配逐板施工页，包含定位、符号、材料表与准确校准尺。"}</p><label className="pdf-mode">图纸结构<select value={pdfLayout} onChange={(e) => setPdfLayout(e.target.value as PdfLayoutMode)}><option value="overview">仅总览版（1 页）</option><option value="construction">完整施工版</option></select></label>{project.guideCells && <label className="pdf-mode">图纸内容<select value={pdfMode} onChange={(e) => setPdfMode(e.target.value as PdfContentMode)}><option value="reference">完整参考图</option><option value="overlay">参考图 + 完成进度</option><option value="progress">仅已填豆子</option></select></label>}{pdfLayout === "overview" && pdfUsedColors > 24 && <small className="pdf-hint">当前使用超过 24 种颜色，完整施工版会附加完整材料清单。</small>}<div className="pdf-facts"><span><small>页面</small><b>{pdfPages}</b></span><span><small>{pdfMode === "progress" ? "已填豆数" : "目标豆数"}</small><b>{pdfMode === "progress" || !project.guideCells ? total : project.guideCells.reduce((sum, value) => sum + (value ? 1 : 0), 0)}</b></span></div><button className="primary wide" onClick={() => exportPdf(project, { contentMode: pdfMode, layoutMode: pdfLayout })}>导出 PDF</button></article>
...
function NewProjectModal({ onClose, onCreate }: { onClose: () => void; onCreate: (w: number, h: number, name: string, board: number) => void }) {
  const [w, setW] = useState(DEFAULT_CREATION_SIZE), [h, setH] = useState(DEFAULT_CREATION_SIZE), [name, setName] = useState("UNTITLED BEAD 01"), [board, setBoard] = useState(29);
  return <div className="modal-backdrop"><section className="modal small-modal" role="dialog" aria-modal="true"><header><div><span className="eyebrow">NEW GRID / 01</span><h2>新建拼豆作品</h2></div><button className="icon-button" onClick={onClose}>×</button></header><div className="new-form"><label>作品名称<input value={name} onChange={(e) => setName(e.target.value)}/></label><CanvasSizePresets width={w} height={h} onSelect={(size) => { setW(size); setH(size); }} /><div className="form-row"><label>宽度<input type="number" min={MIN_CREATION_SIZE} max={MAX_CREATION_SIZE} value={w} onChange={(e) => setW(Math.max(MIN_CREATION_SIZE, Math.min(MAX_CREATION_SIZE, +e.target.value)))}/></label><label>高度<input type="number" min={MIN_CREATION_SIZE} max={MAX_CREATION_SIZE} value={h} onChange={(e) => setH(Math.max(MIN_CREATION_SIZE, Math.min(MAX_CREATION_SIZE, +e.target.value)))}/></label></div><label>拼豆板规格<select value={board} onChange={(e) => setBoard(+e.target.value)}><option value="29">29 × 29 标准板</option><option value="16">16 × 16 小板</option><option value="32">32 × 32 大板</option></select></label><div className="new-summary"><span>{w * h}</span> 个网格 · 预计 {Math.ceil(w / board) * Math.ceil(h / board)} 块拼豆板</div></div><footer><button className="ghost" onClick={onClose}>取消</button><button className="primary" onClick={() => onCreate(w, h, name, board)}>创建画布</button></footer></section></div>;
}
